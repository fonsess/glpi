...
class PluginTagTagItem extends CommonDBRelation {
    ...
    * @return boolean
    */
    //ALTERAÇÃO: atribuição de etiquetas automática de acordo com RequestType e independente de permissões de usuário
    static function updateItem(CommonDBTM $item, bool $delete_existing_tags = true) {
      ...
      */
      ...
      //atualizar tags por setor
      if($item->input["requesttypes_id"] == null) {
      ...
      $newReq = $item->input["requesttypes_id"];
      $oldReq = $item->fields["requesttypes_id"];
      //$tecnicoF = $item->fields["users_id_recipient"];
      //$tecnicoI = $item->input["_actors"];
      //$tec2 = $item->input["_actors"];

      //DEBUG
      $dumpFile = fopen('dump.txt', 'w');
      fwrite($dumpFile, "fields: " . serialize($oldReq) . " --- input: " . serialize($newReq));
      fclose($dumpFile);

      $dumpFile2 = fopen('dump2.txt', 'w');
      fwrite($dumpFile2, serialize($item));
      fclose($dumpFile2);


      if($newReq == null) {
         return true;      //se a requesttype nova for nula, não realizar nenhuma alteração
      }

      if(($newReq != $oldReq) && ($delete_existing_tags)) { 
         $item->input['status'] = "1";    //se a requesttype foi alterada, setar status do chamado para NOVO
         $removed_tags_ids = $existing_tags_ids;      //e colocar tags existentes na lista de remoção
      } else {
         $removed_tags_ids = [];    //se a requesttype não foi alterada, não remover tag
      }

      

      include 'setores.php';
      array_push($tag_values, $setor[$newReq]);

      $added_tags_ids   = array_diff($tag_values, $existing_tags_ids);
      //$removed_tags_ids = $delete_existing_tags ? array_diff($existing_tags_ids, $tag_values) : [];
      //$removed_tags_ids = $delete_existing_tags ? $existing_tags_ids : [];
      ...
      */
      //$requestType 
      //var_dump(['ADD ID: ', $added_tags_ids, 'REMOVED ID: ', $removed_tags_ids, 'TAG VALUE:', $tag_values, 'COMMON ITEM: ', $item]);
      //$dumpContent = "add: " . serialize($added_tags_ids) . " ---- rem: " . serialize($removed_tags_ids) . " ---- existing: " . serialize($existing_tags_ids);
      //$dumpFile = fopen('dump.txt', 'w');
      //fwrite($dumpFile, "old: " . serialize($oldReq) . " ---- new: " . serialize($newReq));
      //fclose($dumpFile);
      //crash();
      //sleep(10);
      //$item->updateInDB("status", [1]);
      return true;
   }
